...
The coordinate-wise differences are $\bm x-\bm y=(1,-1,0,0,0,0)$.
\begin{align*}
L_0(\bm x,\bm y) &= \#\{i:x_i\neq y_i\}=2,\\
...
\begin{proof}[\solutionname]
``Pass'' means the filter accepts (all $k$ probed bits equal $1$); ``block'' means it rejects (at least one probed bit equals $0$).
...
During insertion, each of the $k$ hash functions sends each of the $m$ keys to a uniformly random bit, for a total of $km=300$ independent uniform throws into $n=8$ bins. A particular bit stays $0$ iff none of those throws lands on it, so
\[
\Pr[\text{bit }i=0]=\Bigl(1-\tfrac{1}{n}\Bigr)^{km}=\Bigl(\tfrac{7}{8}\Bigr)^{300},
\qquad
\Pr[\text{bit }i=1]=1-\Bigl(\tfrac{7}{8}\Bigr)^{300}.
\]
...
\[
\boxed{\;\Pr[\text{pass}\mid x\notin S]=\Bigl(1-\bigl(1-\tfrac{1}{n}\bigr)^{km}\Bigr)^{k}=\Bigl(1-\bigl(\tfrac{7}{8}\bigr)^{300}\Bigr)^{3}.\;}
\]
...
\[
\bigl(\tfrac{7}{8}\bigr)^{300}=e^{300\ln(7/8)}\approx e^{-40.06}\approx 4.4\times 10^{-18},
\]
so
\[
\Pr[\text{pass}\mid x\notin S]\approx \bigl(1-4.4\times 10^{-18}\bigr)^{3}\approx 1-1.3\times 10^{-17}\;\approx\;1.
\]
...
\textbf{(b) Flajolet–Martin.} Apply the hash to each distinct stream value. Since $h$ takes values in $\{0,1,\dots,6\}$, we represent each value with $\lceil\log_2 7\rceil=3$ bits and let $r(x)$ denote the number of trailing zeros in the binary expansion of $h(x)$.
\begin{center}
