...
%================================================================
\begin{homework}
\textbf{Problem 1 (12 pts).} Let $P=\{p_1,\dots,p_n\}\subset\MR$ with $p_1\le p_2\le\cdots\le p_n$.
\begin{enumerate}
...
\]
For odd $n$, $2k-n$ is never zero. It is negative for $k\le \frac{n-1}{2}$ and positive for $k\ge \frac{n+1}{2}$. Thus $h$ strictly decreases on $(-\infty,p_{(n+1)/2}]$ and strictly increases on $[p_{(n+1)/2},\infty)$. Hence the unique minimizer over all of $\MR$, and a fortiori the unique minimizer in $P$, is
\[
p^*=p_{(n+1)/2}.\qedhere
\]
\end{proof}
...
%================================================================
\begin{homework}
\textbf{Problem 2 (10 pts).} Let $\mu^*$ be the optimal $k$-center cost (with arbitrary centers in $\MR^d$) and $\widehat\mu^*$ the optimal restricted $k$-center cost (centers must lie in $P$). Prove $\mu^*\le \widehat\mu^*\le 2\mu^*$.
\end{homework}

\begin{proof}\solutionname

\textbf{Lower bound $\mu^*\le \widehat\mu^*$.}
Every restricted clustering is also a feasible (unrestricted) $k$-clustering, since $P\subset\MR^d$. Therefore the optimum over a smaller (restricted) feasible set is at least the optimum over the larger feasible set:
$\mu^*=\min_{\text{all}}\text{cost}\le \min_{\text{restricted}}\text{cost}=\widehat\mu^*.$

\textbf{Upper bound $\widehat\mu^*\le 2\mu^*$.}
Let $\{(c_i^*,C_i^*)\}_{i\in[1,k]}$ be an optimal unrestricted clustering with cost $\mu^*$. For each $i$, choose any data point $\widehat c_i\in C_i^*$ as a restricted center (assume $C_i^*\ne\emptyset$; empty clusters can be discarded). Keep the partition the same. For any $p\in C_i^*$, the triangle inequality gives
\[
...
%================================================================
\begin{homework}
\textbf{Problem 3 (5 pts).} Graph $G=(V,E)$ has three connected components on vertex sets $V_1=\{v_1,\dots,v_{n/3}\}$, $V_2=\{v_{n/3+1},\dots,v_{2n/3}\}$, $V_3=\{v_{2n/3+1},\dots,v_n\}$. Provide three orthonormal eigenvectors of the Laplacian $L$ for eigenvalue $0$.
\end{homework}
...
%================================================================
\begin{homework}
\textbf{Problem 4 (5 pts).} Let $S$ be a finite point set with centroid $\mu(S)$, and let $T$ be disjoint from $S$. Show that
\[
...
\]
The inequality (in fact equality) follows.
\end{proof}

%================================================================
\begin{homework}
\textbf{Problem 5 (8 pts).} For the weighted graph in Figure 1 of the assignment:
\begin{enumerate}
...
The smallest eigenvalue of $L$ is $0$ with eigenvector $\bm 1/\sqrt{5}$. The Fiedler vector $\phi_2$ (second-smallest eigenvalue) approximately separates the two clusters of strongly-connected vertices. Heavy edges of weight $50$ form two tightly bound groups: $A=\{v_1,v_2,v_3\}$ joined through $v_1$, and $B=\{v_4,v_5\}$ joined directly. Light weight-$1$ edges only weakly connect $A$ and $B$.

The Fiedler vector takes opposite signs on $A$ and $B$. To leading order (ignoring small corrections from the weight-$1$ cross edges, which are $50\times$ smaller than intra-cluster edges) it is the indicator-difference vector orthogonal to $\bm 1$. With $|A|=3$, $|B|=2$, requiring $\sum_i\phi_2(i)=0$ and $\|\phi_2\|=1$ gives
\[
\phi_2 \;\approx\; \Bigl(\sqrt{\tfrac{2}{15}},\;\sqrt{\tfrac{2}{15}},\;\sqrt{\tfrac{2}{15}},\;-\sqrt{\tfrac{3}{10}},\;-\sqrt{\tfrac{3}{10}}\Bigr)
\;\approx\;(0.365,\,0.365,\,0.365,\,-0.548,\,-0.548).
\]
...
%================================================================
\begin{homework}
\textbf{Problem 7 (2 pts).} Using Figure 2, augment the signatures with $h_3(x)=2x+1\bmod 5$ and $h_4(x)=3x-1\bmod 5$.
\end{homework}
...
The full signature matrix (rows of given $h_1,h_2$ included for reference) is
\[
\begin{array}{c|cccc}
...
%================================================================
\begin{homework}
\textbf{Problem 8 (8 pts).} For the matrix in Figure 3 with $h_1(x)=2x+1\bmod 6,\;h_2(x)=3x+2\bmod 6,\;h_3(x)=5x+2\bmod 6$:
\begin{enumerate}
...
$h_1$ takes values $\{1,3,5,1,3,5\}$, hitting only odd residues; not a permutation. $h_2$ takes values $\{2,5,2,5,2,5\}$; not a permutation. $h_3$ takes values $\{2,1,0,5,4,3\}$, all six residues exactly once; $h_3$ \emph{is} a true permutation. (Indeed $ax+b\bmod n$ is a permutation iff $\gcd(a,n)=1$, and $\gcd(5,6)=1$ while $\gcd(2,6)=\gcd(3,6)>1$.)

\textbf{(c) Estimated vs.\ true Jaccards.}
True Jaccards from the columns:
\[
\begin{aligned}
J(S_1,S_2)&=0/3=0, & J(S_1,S_3)&=1/4=0.25, & J(S_1,S_4)&=1/3\approx 0.333,\\
